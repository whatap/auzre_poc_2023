...
resource "azurerm_mysql_flexible_server" "petclinic" {
  ...
  name                         = "petclinic-mysqlfs-${var.random_string_name_result}"
  resource_group_name          = azurerm_resource_group.rg.name
  administrator_login          = var.mysqladminloginuser
  administrator_password       = var.mysqladminloginpasswd
  backup_retention_days        = 7
  delegated_subnet_id          = azurerm_subnet.petclinic_subnet_delegated.id
  ...
  lifecycle {ignore_changes = [name, high_availability, maintenance_window, storage, private_dns_zone_id, zone, delegated_subnet_id, sku_name, version]}
}
